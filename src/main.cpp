...
int main(int argc, char *argv[])
{
	...
	// Create a window
	SDL_Window *window = SDL_CreateWindow("Hello SDL", 100, 100, windowWidth, windowHeight,
...
float vertices[] = {
    ...
    unsigned char *data = stbi_load(RESOURCES_PATH"woodSteelCrate.png", &width, &height, &nrChannels, 0);
    if (data)
    {
	...
	stbi_set_flip_vertically_on_load(true); // tell stb_image.h to flip loaded texture's on the y-axis.
	// The FileSystem::getPath(...) is part of the GitHub repository so we can find files on any IDE/platform; replace it with your own image path.
	data = stbi_load(RESOURCES_PATH"woodSteelCrateSpecularMap.png", &width, &height, &nrChannels, 0);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
		glGenerateMipmap(GL_TEXTURE_2D);
